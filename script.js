document.addEventListener('DOMContentLoaded', () => {
    ...
    // Reusable article card rendering
    ...
    // Track overlay stack to manage nested overlays
    const overlayStack = [];

    // Modified resetOverlays to selectively close overlays without resetting everything
    ...
    // Render individual post in overlay
    const renderPostOverlay = (postId) => {
        ...
        `;
        resetOverlays(articleOverlay); // Keep article overlay open
    ...
    const renderSavedArticles = () => {
        ...
        `;
        resetOverlays(savedArticlesOverlay); // Keep saved articles overlay open
...
// Toggle search overlay and button
if (searchBtn && searchOverlay) {
    ...
    searchBtn._searchHandler = (e) => {
        ...
        resetOverlays(searchOverlay); // Close other overlays
        overlayStack.push(searchOverlay);
        searchOverlay.classList.toggle('active');
        searchBtn.classList.toggle('active');
        if (searchOverlay.classList.contains('active')) {
            console.log('Focusing search input'); // Debug
            setTimeout(() => searchInput.focus(), 100); // Ensure focus after overlay opens
            populateSearchResults();
        } else {
            searchBtn.classList.remove('active'); // Ensure button resets if overlay closed
        }
    };
...
if (closeSearch) {
    ...
    closeSearch._closeHandler = () => {
        console.log('Close search clicked'); // Debug
        searchOverlay.classList.remove('active');
        searchBtn.classList.remove('active'); // Reset search button
        overlayStack.pop();
        if (overlayStack.length > 0) {
            const previousOverlay = overlayStack[overlayStack.length - 1];
            previousOverlay.classList.add('active');
...
if (searchForm) {
    searchForm.addEventListener('submit', (e) => {
        e.preventDefault();
        console.log('Search form submitted'); // Debug
        populateSearchResults();
    });
}
...
if (searchOverlay) {
    ...
    const firstFocusable = focusableElements[0];
    const lastFocusable = focusableElements[focusableElements.length - 1];
    console.log('Focusable elements:', Array.from(focusableElements).map(el => el.outerHTML)); // Debug
...
function populateSearchResults() {
    ...
    searchResultsGrid.querySelectorAll('.article-card').forEach(card => {
        ...
        card._clickHandler = (e) => {
            if (!e.target.closest('.save-btn, .share-btn')) {
                ...
                console.log('Article card clicked:', postId); // Debug
                renderPostOverlay(postId);
            }
        };
    ...
    const showPage = (page) => {
        ...
        // Add animation classes to articles-grid
    ...
    const renderPagination = (totalPages) => {
            ...
            if (!isDisabled) {
                btn.addEventListener('click', () => {
                    ...
                    if (articlesGrid) {
                        ...
                        window.scrollTo({ 
                            top: offsetTop - 100, // Scroll to 100px above articles-grid
                            behavior: 'smooth' 
                        });
                    }
                });
        ...
        // Show only 3 page numbers, shifting as needed
